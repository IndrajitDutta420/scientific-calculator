...
display.addEventListener('keydown', function(event) {
    // 1. Allow functional keys (Backspace, Arrow keys, Delete, Tab) and shortcuts (Ctrl+C)
    if (event.key.length > 1 || event.ctrlKey || event.metaKey) {
        return; 
    }

    // 2. EXPLICITLY ALLOW THE COMMA (Guarantees it works on all keyboards)
    if (event.key === ',') {
        return;
    }

    // 3. Define exactly what other single characters are allowed
    const allowedCharacters = /^[0-9\+\-\*\/\.\(\)\^!%=]$/;

    // 4. Block anything that is not in the allowed list
